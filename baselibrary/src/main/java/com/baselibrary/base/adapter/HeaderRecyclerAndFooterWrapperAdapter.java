...
public abstract class HeaderRecyclerAndFooterWrapperAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (mHeaderDatas.get(viewType) != null) {//不为空，说明是headerview
            ...
            View headView = LayoutInflater.from(UIUtils.getContext()).inflate(mHeaderDatas.get(viewType).keyAt(0), null);
    ...
    protected abstract void onBindHeaderHolder(BaseViewHolder holder, int headerPos, int layoutId, Object o);//多回传一个layoutId出去，用于判断是第几个headerview

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (isHeaderViewPos(position)) {
            int layoutId = mHeaderDatas.get(getItemViewType(position)).keyAt(0);
            onBindHeaderHolder((BaseViewHolder) holder, position, layoutId, mHeaderDatas.get(getItemViewType(position)).get(layoutId));
            return;
        } else if (isFooterViewPos(position)) {
            return;
        }
        //举例子，2个header，0 1是头，2是开始，2-2 = 0
        Logger.i("TAG",holder.getLayoutPosition() +"---"+getHeaderViewCount());
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position, List<Object> payloads) {
        if (isHeaderViewPos(position)) {
            int layoutId = mHeaderDatas.get(getItemViewType(position)).keyAt(0);
            Logger.e("TAG","header:"+position +"---"+getHeaderViewCount()+"---"+getInnerItemCount()+"---"+getItemCount());
            if(payloads.size()>0) {
                onBindHeaderHolder((BaseViewHolder) holder, position, layoutId, payloads.get(position));
            }else
            {
                onBindHeaderHolder((BaseViewHolder) holder, position, layoutId, mHeaderDatas.get(getItemViewType(position)).get(layoutId));
            }
        ...
        //举例子，2个header，0 1是头，2是开始，2-2 = 0
        Logger.e("TAG","2哈:"+position +"---"+getHeaderViewCount()+"---"+getInnerItemCount()+"---"+getItemCount());
        if(position<getItemCount()-1)
        {
            mInnerAdapter.onBindViewHolder(holder, position);
        }
